...
const countBox = document.getElementById("gbCount");
const searchBox = document.getElementById("gbSearch");
let latestDocs = [];
...
const render = (docs) => {
  ...
  const filtered = docs.filter(({data}) => {
    const name = String(data.nickname || "").toLowerCase();
    const content = String(data.message || "").toLowerCase();
    return !keyword || name.includes(keyword) || content.includes(keyword);
  });

  if (countBox) countBox.textContent = `${docs.length} 条留言`;

  if (!filtered.length) {
    list.innerHTML = `<div class="gb-empty">${docs.length ? "没有找到匹配的留言。" : "暂时还没有留言，留下第一条吧。"}</div>`;
    return;
  }
...
searchBox?.addEventListener("input", () => render(latestDocs));
setInterval(() => render(latestDocs), 60000);
